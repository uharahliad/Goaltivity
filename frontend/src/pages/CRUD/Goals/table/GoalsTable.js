...
const GoalsTable = () => {
  ...
  const columns = [
      ...
      { field: "category",
        ...
        sortable: false,
        renderCell: (params) => goal_categoriesDataFormat.listFormatter(params.row[params.field], history, 'goal_categories'),
        flex: 1,
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="goals" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('Goals')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/goals/${e.id}/edit`)}}
            autoHeight
          />
        </div>
