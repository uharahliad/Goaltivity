...
const Goal_categoriesTable = () => {
  ...
  const modalOpen = useSelector((store) => store.goal_categories.list.modalOpen);
  const rows = useSelector((store) => store.goal_categories.list.rows);
  const idToDelete = useSelector((store) => store.goal_categories.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="goal_categories" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('Goal_categories')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/goal_categories/${e.id}/edit`)}}
            autoHeight
          />
        </div>
