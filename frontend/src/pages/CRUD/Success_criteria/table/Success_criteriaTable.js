...
const Success_criteriaTable = () => {
  ...
  const modalOpen = useSelector((store) => store.success_criteria.list.modalOpen);
  const rows = useSelector((store) => store.success_criteria.list.rows);
  const idToDelete = useSelector((store) => store.success_criteria.list.idToDelete);
  ...
  const columns = [
      ...
      { field: "goal",
        ...
        sortable: false,
        renderCell: (params) => goalsDataFormat.listFormatter(params.row[params.field], history, 'goals'),
        flex: 1,
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="success_criteria" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>{humanize('Success_criteria')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/success_criteria/${e.id}/edit`)}}
            autoHeight
          />
        </div>
